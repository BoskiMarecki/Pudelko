...
namespace PudelkoTests
{
    ...
    public class UnitTestsPudelkoConstructors
    {
        ...
        [DataTestMethod, TestCategory("Pole, Objetosc")]
        [DataRow(3, 4, 5, UnitOfMeasure.meter, 60)]
        [DataRow(45, 25, 30, UnitOfMeasure.centimeter, 0.03375)]
        [DataRow(465, 736, 995, UnitOfMeasure.milimeter, 0.3405288)]
        [DataRow(999, 999,123, UnitOfMeasure.milimeter, 0.122754123)]
        public void ObjetoscTest(double a, double b, double c, UnitOfMeasure unit, double expected)
        {
        ...
        [DataRow(12,13,14, UnitOfMeasure.centimeter,0.1012)]
        [DataRow(123,456,789, UnitOfMeasure.milimeter,1.025838)]

        public void PoleTest(double a, double b, double c, UnitOfMeasure unit, double area)
        {
            var x = new Pudelko(a, b, c, unit);
            Assert.AreEqual(area, x.Pole);
        }
        ...
        [DataRow(1.001, 2.02, 3, 1, 1, 4.021,true)]
        [DataRow(1,2,3,1,2,3,true)]
        [DataRow(4, 5, 6, 7, 8, 2, false)]
        public void EqualsTest(double a1, double a2, double a3, double b1, double b2, double b3, bool expected)
        {
            Pudelko p1 = new Pudelko(a1, a2, a3);
            Pudelko p2 = new Pudelko(b1, b2, b3);
        ...
        [DataRow(4, 5, 6, 7, 8, 3, true)]
        public void EqualsTest1(double a1, double a2, double a3, double b1, double b2, double b3, bool expected)
        {
            Pudelko p1 = new Pudelko(a1, a2, a3);
            Pudelko p2 = new Pudelko(b1, b2, b3);
            Assert.AreEqual(expected, p1 != p2);
        ...
        [DataRow(1, 2, 3, 4, 5, 6, 5, 5, 6)]
        [DataRow(2, 2, 2, 3, 3, 3, 5, 3, 3)]

        public void AdditionTest(double a1, double a2, double a3, double b1, double b2, double b3, double e1, double e2, double e3)
        {
            ...
            Pudelko expectedPudelko = new Pudelko(e1, e2, e3);

            Assert.AreEqual(expectedPudelko.A, p3.A);
            Assert.AreEqual(expectedPudelko.B, p3.B);
            Assert.AreEqual(expectedPudelko.C, p3.C);
        }
        #endregion
        ...
        [DataRow("1.00 m x 2.000 m x 3.000", 1, 2.0,3.00, UnitOfMeasure.meter)]
        [DataRow("2.9 m x 3.8 m x 4.7", 2.908, 3.80, 4.710, UnitOfMeasure.centimeter)]
